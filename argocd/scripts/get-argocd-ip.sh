#!/usr/bin/env bash
# get-argocd-ip.sh — poll for the ArgoCD LoadBalancer IP and return it as JSON
#
# Called by the Terraform external data source in argocd.tf.
# Reads a JSON query from stdin containing the cluster kubeconfig,
# polls kubectl until the LoadBalancer IP is assigned, then writes
...
# Poll until the LoadBalancer IP is assigned by Azure (up to 5 min)
for i in $(seq 1 30); do
  ...
  echo "Attempt $i/30 — waiting 10s for ArgoCD LoadBalancer IP..." >&2
  sleep 10
done
